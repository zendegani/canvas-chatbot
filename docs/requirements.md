...
- **Multi-Model Comparison**: Users can select up to two models on a single node. Sending a prompt forks the conversation into two side-by-side children, allowing instant comparison of models.
...
### 2.5 Chat History & Navigation

- **Collapsible Sidebar**: A unified interface containing navigation, chat history, user settings, and workspace controls.
- **Auto-Save & Sessions**: Ongoing canvases are automatically saved as sessions. The app stores up to 50 local sessions per user, evicting the oldest when the limit is reached.
- **Seamless Context Switching**: Users can seamlessly load past conversations visually recreating the complete node branching structure.
...
- **Persistence**: `localStorage` used for persisting:
  - `canvasSessions_{user}`: The index/metadata of up to 50 saved sessions.
  - `canvasSession_{user}_{id}`: The full chat graph and nodes for a specific session.
